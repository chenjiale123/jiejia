    ...
    <div v-if="homeActive==2">
      ...
      <div class="stepThree">
        ...
        <div class="tips">
          ...
          <div>3、 单张图片：不超过1024KB，所有图片不超过1024KB</div>
...
import { saveGoodsInfo, updateInfoDetails } from '../../../api/homeMarket/shop';
...
export default {
  ...
  created() {
    ...
    if (this.id) {
      this.baseInfo = this.$store.state.homeBaseInfo;
    ...
    addShop() {
      ...
      if (this.id) {
        ...
        updateInfoDetails(this.baseInfo).then(res => {
          if (res.code === 200) {
            that.$message('修改成功');
            that.$emit('update:isShowEdit', false);
          }
        });
      } else {
        const baseInfo = this.$store.state.homeBaseInfo;
        baseInfo.services = this.$store.state.homeBaseInfo.services.toString();
        baseInfo.mainImg = this.$store.state.homeBaseInfo.mainImg.toString();
        const goodsDetails = this.$store.state.homeGoodDetail;
        const describe = {
        ...
        saveGoodsInfo(dataInfo).then((res) => {
          if (res.code === 200) {
            that.$message('添加成功');
            setTimeout(function () {
              that.$router.push({ path: '/homeMarket/homeShopList' });
              that.$store.commit('changActive', 0);
            }, 1000);
          } else {
