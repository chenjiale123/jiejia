      ...
      <div class>
        <el-row>
          <el-col :span="18">
            <el-form-item label="登录账号:" prop="loginid">
              <el-input v-model="form.loginid" placeholder="请输入登录账号" maxlength="50" :disabled="id!==undefined" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          <el-col :span="18">
           ...
           <el-select v-model="form.store" placeholder="请选择" :disabled="true">
...
export default {
  data() {
    return {
      ...
      form: {
          store: Number(localStorage.getItem("storeId")) 
      },
      storeTree: [],
      isDisable: false,
      // 表单校验
  ...
  methods: {
      ...
      getStoreDetail({ userId: id })
        .then(res => {
          ...
          if (res.code === 200) {
            ...
            this.form.store=Number(res.data.storeIds)
    ...
    cancel() {
      this.$router.push({ path: '/storeUser/findStoreUserList' });
    },
    submitForm() {
      if (this.id) {
        console.log(this.arr);
