<template>
  <div class="wrap">
    <div v-if="id" class="editInfo">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClickTab" v-if="!isEdit">
          ...
          <el-tab-pane label="业务规格" name="second">
            ...
            <div class="specifiTable">
              <el-table :data="tableDatas" v-loading="load">
                <el-table-column
                  label="商品名称"
                  align="center"
                  prop="name"
                ...
                >
                  <template slot-scope="scope">
                    <img :src="scope.row.specImg" alt width="50px" />
                  </template>
                </el-table-column>
                <el-table-column
                  label="状态"
      ...
      <addShopBaseInfo v-if="isEdit&&activeName=='first'" :isShowEdit.sync="isEdit"></addShopBaseInfo>
      <addSpecifications v-if="isEdit&&activeName=='second'" :isShowEdit.sync="isEdit"></addSpecifications>
      <addShopDetail v-if="isEdit&&activeName=='third'" :isShowEdit.sync="isEdit"></addShopDetail>
    </div>

    <div v-else>
      <step :homeActive="homeActive"></step>
...
<script>
import step from '../../common/homeMarket/addBaseInfo';
import addShopBaseInfo from '../../common/homeMarket/addShopBaseInfo';
import addSpecifications from '../../common/homeMarket/addSpecifications';
import addShopDetail from '../../common/homeMarket/addShopDetail';
import { findInfoDetails, findGoodsInfoDetails } from '../../../api/homeMarket/shop';
export default {
  data() {
    return {
      id: null,
    ...
    listEdit(id) {
      findInfoDetails(id).then((res) => {
        if (res.code === 200) {
          ...
          this.$store.dispatch('getHomeBaseInfo', this.baseInfos);
          console.log(this.baseInfos.services, '909090000');
        }
      });
    },
    // 获取商品列表
    getGoodsList() {
      ...
      findGoodsInfoDetails({ id: this.id, classId: this.classId }).then(res => {
        if (res.code === 200) {
          ...
          this.load = false;
          this.$store.dispatch('getHomeGoodsDetail', this.tableDatas);
        }
      });
    },
    // 点击编辑
