          ...
          <el-tab-pane label="业务规格" name="second">
            ...
            <div class="specifiTable">
              <el-table :data="tableDatas">
                ...
                <el-table-column property="stock" label="库存" v-if="state==2">
                  <template slot-scope="scope">
                    <el-input
                    ...
                    ></el-input>
                    <span v-if="!hidden">{{scope.row.stock}}</span>
                    <span @click="edit(scope.row)"  style="color:#409EFF;cursor:pointer">修改</span>
                  </template>
                </el-table-column>
              ...
              <div slot="footer" class="dialog-footer" v-if="hidden">
...
export default {
  ...
  watch: {
    isEdit: {
      handler: function(val, oldval) {
        ...
        this.listEdit(this.id, localStorage.getItem("storeId"));
  ...
  created() {
    ...
    this.id = this.$route.query.goodsInfoId;
    ...
    if (this.id) {
      this.listEdit(this.id, this.storeId);
      this.getGoodsList();
    }
  ...
  methods: {
    ...
    remain() {
      ...
      for (let i in this.tableDatas) {
        arr.push({
          goodsInfoId: this.tableDatas[i].goodsInfoId,
          stock: Number(this.tableDatas[i].stock),
          storeId: Number(localStorage.getItem("storeId")),
          goodsDetailId: Number(this.tableDatas[i].detailId),
          goodsStatus: Number(this.tableDatas[i].status)
        });
      }
      editStockMain(arr).then(res => {
        console.log(res);
        if (res.code === 200) {
          this.msgSuccess("修改库存成功");
          this.hidden = false;
        }
      });
    },
    ...
    getGoodsList() {
      ...
      findGoodsInfoDetails({
        ...
        if (res.code === 200) {
          this.tableDatas = res.data.marketGoodsDetailList;

          if (res.data.marketGoodsDetailList.length > 0) {
