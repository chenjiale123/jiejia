      ...
      <el-table :data="goodsList" v-loading="loading" :max-height="clieHeight">
        <el-table-column label="商品图片" align="center" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            ...
            <img :src="scope.row.mainImg.split(',')[0]" alt class="mainImg" />
          </template>
        </el-table-column>
...
<script>
import { getGoodsSpec, getGoodsInfoList, deleteGoodsInfo, updateGoodsStatus, getClassifyTree, getClassifyDetail } from '../../../api/business/index';
export default {
  data() {
    return {
      queryParam: {
  ...
  methods: {
    ...
    // 添加
    handleAdd() {
      this.$router.push({ path: '/business/addShop' });
      this.$store.commit('changState', 0);
    },
    toDetail(id, classId) {
      console.log(id, '======');
      this.$router.push({ path: '/business/addShop', query: { id: id, classId: classId } });
    },
    handleDel(id) {
      const that = this;
