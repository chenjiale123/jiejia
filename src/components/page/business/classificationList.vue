...
import { getClassifyTree, addClassify, getClassifyDetail, updClassify, delClassify, updClassifyStatus } from '@/api/business/index';
export default {
  ...
  methods: {
    ...
    lookClass(item) {
      console.log(item);
      this.$router.push({ path: '/business/specificationsList', query: { id: item.id } });
    },
    lookShop(item) {
      console.log(item);
      this.$router.push({ path: '/business/findShopList', query: { id: item.id } });
    }
  }
};
</script>
